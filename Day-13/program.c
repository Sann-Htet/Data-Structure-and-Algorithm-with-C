...
void record_data(){
    ...
    for(int i=0; i<g_userCount; i++){
        data[i].user_id=i;
        fprintf(fptr,"%d%c%s%c%s%c%s%c%d%c%s%c%d%c",data[i].user_id,' ',data[i].user_name,' ',
                data[i].user_email,' ',data[i].user_pass,' ',data[i].phone_number,' ',data[i].address,' ',data[i].postal_code,'\n');
...
void loading_data_from_file(){
    ...
    for(int z=0; z<USERSIZE; z++) {
        fscanf(fptr, "%d%s%s%s%d%s%d",
               &data[z].user_id, &data[z].user_name, &data[z].user_email, &data[z].user_pass,
               &data[z].phone_number, &data[z].address, &data[z].postal_code);
...
void all_data(){
    ...
    for(int z=0; z<g_userCount;z++){
        printf("%d%c%s%c%s%c%s%c%d%c%s%c%d\n",data[z].user_id,'-',data[z].user_name,'-',data[z].user_email,'-',data[z].user_pass,'-',data[z].phone_number,'-',data[z].address,'-',data[z].postal_code);
    }
